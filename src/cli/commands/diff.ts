...
import { formatDiffAnnotated, formatDiffAsJSON, formatDiffAsText } from '~~/formats';
...
export default function diff(program: Command) {
    program
        ...
        .action(async (firstInput: string, secondInput: string, options: { format: 'text' | 'annotated' | 'json' }) => {
            ...
            await tryCatch(async () => {
                ...
                const source1: Uint8Array | AssemblyProgram =
                    firstResult.format === 'bytecode'
                        ? hexToBytes(firstResult.content.trim())
                        : { lines: parseAssemblyFile(firstResult.content), warnings: [] };
                const source2: Uint8Array | AssemblyProgram =
                    secondResult.format === 'bytecode'
