...
import { colorizeImmediate, colorizeOpcode } from '~~/formats/colors';
import { FunctionIdentifier } from '~~/libraries/fi';
import { hexToBytes } from '~~/utils';
import { GasCalculator } from '~~/utils/gas-calculator';
import { error, hint, sectionFooter, sectionHeader, startSpinner, stopSpinner, success, tryCatch } from '../utils';
    ...
    program
        ...
        .action(async (input: string, options: { top: string; window: string; functions: boolean }) => {
            await tryCatch(async () => {
                ...
                const topOpcodes = [...report.byOpcode.entries()].sort((a, b) => b[1].gas - a[1].gas).slice(0, 10);
