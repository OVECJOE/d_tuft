...
import { error, hint, kv, parseAssemblyFile, sectionFooter, sectionHeader, success, tryCatch } from '../utils';
    ...
    program
        ...
        .action(async (input, options) => {
            await tryCatch(async () => {
                ...
                console.error(
                    T.chrome.border(
                        `  ┌─ ${lines.length} instruction(s) ${'─'.repeat(Math.max(0, 44 - String(lines.length).length))}`,
                    ),
                );
