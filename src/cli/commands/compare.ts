...
import {
    ...
    type CompareFormat, startSpinner, stopSpinner,
    success, error, info, hint, sectionHeader, sectionFooter, kv, box
} from "../utils";
...
export default function compare(program: Command) {
    program
        ...
        .action(async (file1: string, file2: string, options: { format: 'auto' | CompareFormat } = { format: 'auto' }) => {
            ...
            await tryCatch(async () => {
                ...
                if (differences === 0) {
                    console.log(box(`${T.status.success('✓')} Files are equivalent — zero differences found.`, 'MATCH'));
                } else {
                    console.log(box(`${T.status.error('✗')} ${differences} difference(s) found between the two files.`, 'MISMATCH'));
                    process.exit(1);
                }
            });
        });
