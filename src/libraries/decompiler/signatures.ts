...
const KNOWN_PATTERNS: PatternDefinition[] = [
    ...
    },
    {
        ...
        selectors: ['0x8456cb59', '0x01ffc9a7', '0x5c975abb'],
        storageLayout: [0],
    },
    {
        name: 'UniswapV2Pair',
    ...
    },
    {
        ...
        selectors: ['0xac9650d8', '0x5ae401dc', '0x47b97e19'],
    },
    {
        name: 'Proxy (ERC1967)',
        version: 'EIP-1967',
        description: 'Transparent upgradeable proxy',
        selectors: ['0x3593564c', '0xac9650d8'],
    },
];

export class SignatureMatcher {
    match(selectors: string[], _storageSlots: number[]): MatchedPattern[] {
        const results: MatchedPattern[] = [];

        for (const pattern of KNOWN_PATTERNS) {
            const matchedSelectors = pattern.selectors.filter((s) => selectors.includes(s.toLowerCase()));
