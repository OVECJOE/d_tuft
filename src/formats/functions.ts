...
export function formatFunctionsAsText(maps: FunctionMap[]): string {
    if (maps.length === 0) {
        return chalk.yellow(
            'No public functions found. Contract may be a library, proxy, or use custom dispatch logic.',
        );
    }
    ...
    for (const fn of maps) {
        const label = fn.name ? chalk.green(fn.name) : chalk.gray(`<unknown> ${fn.selector}`);
...
export function formatFunctionsAnnotated(maps: FunctionMap[]): string {
    ...
    for (const fn of maps) {
        const header = fn.name ? `╔══ ${fn.name} ══ ${fn.selector}` : `╔══ <unresolved> ══ ${fn.selector}`;
