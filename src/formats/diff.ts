...
import type { FunctionMap, Instruction, SelectorDiff } from '../core/types';
...
export function formatDiffAnnotated(
    ...
    for (const diff of diffs) {
        const label = diff.name ? chalk.cyan(diff.name) : chalk.gray('<unknown>');
 ...
 */
function formatAnnotatedBody(body: Instruction[], _sourceLabel: string): string {
    const lines: string[] = [];
