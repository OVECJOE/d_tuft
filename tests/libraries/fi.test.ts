...
describe('FunctionIdentifier', () => {
    test('identifies functions from bytecode with dispatcher', () => {
        ...
        const fi = new FunctionIdentifier(new Uint8Array(bytecode.match(/.{2}/g)!.map((b) => parseInt(b, 16))));
    ...
    test('diff detects added functions', () => {
        ...
        const bytecode2 =
            '0x608060405234801561001057600080fd5b50600436106100415760003560e01c806306fdde0314610046578063095ea7b31461006457806318160ddd14610084575b600080fd5b';
        const fi1 = new FunctionIdentifier(new Uint8Array(bytecode1.match(/.{2}/g)!.map((b) => parseInt(b, 16))));
        const fi2 = new FunctionIdentifier(new Uint8Array(bytecode2.match(/.{2}/g)!.map((b) => parseInt(b, 16))));
    ...
    test('diff returns empty for identical bytecode', () => {
        const bytecode =
            '0x608060405234801561001057600080fd5b50600436106100415760003560e01c806306fdde0314610046578063095ea7b31461006457806318160ddd14610084575b600080fd5b';
        const bytes = new Uint8Array(bytecode.match(/.{2}/g)!.map((b) => parseInt(b, 16)));
